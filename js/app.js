import{parse,parseEditedLine,renderLine}from'./parser.js';import{png}from'./exporter.js';import{updateProject,newProject,autoNameFromFormatter}from'./projects.js';
...
function preview(){let raw=$('#filtered').value,lines=raw.replace(/\r/g,'').split('\n').filter(x=>x.trim()).map(x=>parseEditedLine(x,state.mode,$('#self').value.trim())),p=$('#preview');p.innerHTML='';p.style.maxWidth=$('#width').value+'px';p.style.padding=$('#chatbg').checked?'8px':'0';p.style.background=$('#chatbg').checked?'rgba(0,0,0,.45)':'transparent';lines.forEach(l=>{let d=document.createElement('div');d.className='chat';d.style.fontSize=$('#font').value+'px';if($('#timestamps').checked&&l.ts){let s=document.createElement('span');s.className='ts';s.textContent=`[${l.ts}] `;d.appendChild(s)}for(const g of renderLine(l,state.mode,$('#self').value.trim())){let sp=document.createElement('span');sp.style.color=g.color;sp.textContent=g.text;d.appendChild(sp)}d.style.fontFamily=`${$('#fontfamily').value}, Arial, sans-serif`;d.style.fontWeight=$('#fontweight').value;let ol=$('#outline').value;d.style.textShadow=ol==='0'?'none':ol==='1'?'-1px -1px 0 #000,1px -1px 0 #000,-1px 1px 0 #000,1px 1px 0 #000':'-2px -2px 0 #000,2px -2px 0 #000,-2px 2px 0 #000,2px 2px 0 #000,0 2px 0 #000,2px 0 0 #000';p.appendChild(d)});$('#stats').textContent=`${lines.length} lines`;$('#editstatus').textContent=raw===generated?'':'Edited'}
...
$('#newproject').onclick=()=>{if(confirm('Start a new project? Your current work stays in History.')){saveFormatter();newProject();clearFormatterOnly();location.reload()}};

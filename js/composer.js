...
function defaults(text='',extra={}){return{id:uid(),name:`Segment ${segments.length+1}`,text,mode:'auto',self:'',font:16,width:700,opacity:0,timestamps:true,visible:true,left:35,top:35,...extra}}
...
function loadControls(){
 ...
 ['segmentname','chattext','cmode','cself','cfont','chatwidth','opacity','ctimestamps','segmentvisible','duplicatesegment','deletesegment','centerchat'].forEach(id=>$('#'+id).disabled=disabled);
 if(!s){$('#segmentname').value='';$('#chattext').value='';return}
 $('#segmentname').value=s.name;$('#chattext').value=s.text;$('#cmode').value=s.mode;$('#cself').value=s.self;$('#cfont').value=s.font;$('#chatwidth').value=s.width;$('#opacity').value=s.opacity;$('#ctimestamps').checked=s.timestamps;$('#segmentvisible').checked=s.visible;
...
function syncSelected(){
 ...
 s.name=$('#segmentname').value||'Untitled segment';s.text=$('#chattext').value;s.mode=$('#cmode').value;s.self=$('#cself').value;s.font=+$('#cfont').value;s.width=+$('#chatwidth').value;s.opacity=+$('#opacity').value;s.timestamps=$('#ctimestamps').checked;s.visible=$('#segmentvisible').checked;
...
async function restore(){
 ...
 if(incoming?.text){addSegment({text:incoming.text,mode:incoming.mode||'auto',self:incoming.self||'',timestamps:incoming.timestamps!==false,font:+incoming.font||16,name:`Segment ${segments.length+1}`});localStorage.removeItem('gtawComposerIncoming')}
...
['cmode','ctimestamps','segmentvisible'].forEach(id=>$('#'+id).addEventListener('change',syncSelected));
['cfont','chatwidth','opacity'].forEach(id=>$('#'+id).addEventListener('input',syncSelected));
...
$('#export').onclick=async()=>{
 let W=canvas.offsetWidth,H=canvas.offsetHeight,c=document.createElement('canvas');c.width=W;c.height=H;let ctx=c.getContext('2d');ctx.fillStyle='#15191e';ctx.fillRect(0,0,W,H);

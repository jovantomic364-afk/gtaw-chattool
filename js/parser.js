export const C={purple:'#C2A2DA',white:'#F0F0F0',ooc:'#696969',yellow:'#FFFF00',pink:'#FF00C3',grey:'#B8B8B8',green:'#00FF00',orange:'#FFA500',low:'#A6A6A6',lower:'#7F7F7F'};
...
function assistant(body,self){const classify=body.replace(/%([^%]+)%/g,'$1').replace(/\/\/([^/]+)\/\//g,'$1');if(/^\s*You\s+(?:paid|gave|received)\b/i.test(classify)||/^\s*.+?\s+paid\s+you\s+\$[\d,]+\b/i.test(classify)||/^\s*.+?\s+gave\s+you\b/i.test(classify))return[{text:body,color:C.green}];if(/^\s*(?:\(Phone\)\s*)?\*\s+/i.test(body)||/^\s*>\s+/.test(body))return[{text:body,color:C.purple}];if(/^\s*\(\(.*\)\)\s*$/.test(body))return[{text:body,color:C.ooc}];if(/^\s*\[!\]\s*/.test(body))return[{text:'[!] ',color:C.pink},{text:body.replace(/^\s*\[!\]\s*/,''),color:C.white}];if(/\bsays(?:\s+\[[^\]]+\])?\s+\(phone\):/i.test(body)){let sp=(body.match(/^\s*(.+?)\s+says/i)||[])[1]||'';return[{text:body,color:self&&sp.toLowerCase()===self.toLowerCase()?C.white:C.yellow}]}if(/^\s*\[PHONE\]/i.test(body))return[{text:body,color:C.yellow}];if(/\bMessage\s+(?:from|sent to)\s+/i.test(body))return[{text:body,color:C.orange}];if(/\bsays\s+\[lower\](?:\s+\([^)]*\))?:/i.test(body))return[{text:body,color:C.lower}];if(/\bsays\s+\[low\](?:\s+\([^)]*\))?:/i.test(body))return[{text:body,color:C.low}];if(/\bwhispers(?:\s+\[[^\]]+\])?(?:\s+\([^)]*\))?:/i.test(body))return[{text:body,color:C.orange}];return[{text:body,color:C.white}]}
function typeOf(p){if(/^\s*\(\(/.test(p))return'ooc';if(/^\s*(?:\(Phone\)\s*)?\*|^\s*>/.test(p))return'emote';if(/\b(?:says|whispers|shouts)\b/i.test(p))return'speech';if(/\bMessage\s+(?:from|sent to)\s+/i.test(p))return'message';return'system'}
function cleanName(s){return(s||'').replace(/^\[!\]\s*/,'').trim()}
function peopleOf(p){let out=[];let speech=p.match(/^\s*(?:\[!\]\s*)?(.+?)\s+(?:says|whispers|shouts)(?:\s+\[[^\]]+\])?(?:\s+\([^)]*\))?:/i);if(speech)out.push(cleanName(speech[1]));let emote=p.match(/^\s*(?:\(Phone\)\s*)?(?:\*|>)\s+([A-Z][\w'-]+(?:\s+[A-Z][\w'-]+)+)\b/);if(emote)out.push(cleanName(emote[1]));return[...new Set(out.filter(Boolean))]}
...
function inlineEmotes(groups,body){
  ...
  // The marker asterisks are editor syntax only and are not rendered/exported.
  // We only inspect text after the speech colon, so stars elsewhere are left untouched.
  const plain=groups.map(g=>g.text||'').join('');
  const speech=plain.match(/^([\s\S]*?\b(?:says|whispers|shouts)(?:\s+\[[^\]]+\])?(?:\s+\([^)]*\))?:)([\s\S]*)$/i);
  ...
  for(const g of groups){
    ...
    for(let i=0;i<text.length;i++,absolute++){
      ...
      if(afterPrefix&&ch==='*'){
        ...
        if(!inEmote){
          if(remainingStars>=2){flush();inEmote=true;chunkColor=C.purple}else{chunk+=ch}
        }else{flush();inEmote=false;chunkColor=g.color}
        remainingStars--;
      }else{
